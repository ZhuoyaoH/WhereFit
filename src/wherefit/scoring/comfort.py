...
def score_temperature(metrics: ClimateMetrics, pref: UserPreference) -> float:
    ...
    hot_day_penalty = _monthly_rate(metrics.hot_days, metrics) * (0.6 + pref.heat_sensitivity * 0.25)
...
def score_precipitation(metrics: ClimateMetrics, pref: UserPreference) -> float:
    rain_days = _monthly_rate(metrics.precipitation_days, metrics)
    heavy_rain_days = _monthly_rate(metrics.heavy_rain_days, metrics)
    rain_penalty = rain_days * (1.5 + 0.4 * pref.rain_sensitivity)
    heavy_penalty = heavy_rain_days * (4.0 + 0.8 * pref.rain_sensitivity)
...
def _monthly_rate(value: float, metrics: ClimateMetrics) -> float:
    """Convert full-year day counts to the monthly rate expected by the heuristic."""

    return float(value) / max(1, int(metrics.period_months))
