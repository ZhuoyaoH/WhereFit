...
def score_heat_risk(metrics: ClimateMetrics, pref: UserPreference) -> float:
    risk = _monthly_rate(metrics.hot_days, metrics) * (4.8 + pref.extreme_weather_sensitivity * 0.4)
...
def score_heavy_rain_risk(metrics: ClimateMetrics, pref: UserPreference) -> float:
    ...
    precipitation_days = _monthly_rate(metrics.precipitation_days, metrics)
    risk = heavy_rain_days * (15.0 + pref.extreme_weather_sensitivity * 1.2)
    risk += max(0.0, precipitation_days - 12.0) * 1.8
    return normalize_score(risk)
